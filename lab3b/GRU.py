...
from lab3b.layers.GRULayer import GRULayer
...
def main():
    model = Model([
        GRULayer(input_size=16, hidden_size=32, output_size=32, return_sequence=True, wScale=0.4),
        ReLU(),
        GRULayer(input_size=32, hidden_size=16, output_size=12, return_sequence=False, wScale=0.4),
        ReLU(),
        DenseLayer(12, 3),
        Softmax()
    ])
    ...
    X_Seq = sliding_window_view(x, window_shape=(seq_len, i_num)).reshape(-1, seq_len, i_num)
    y_Seq = to_categorical(y[seq_len - 1:], o_num)
    X_train, X_test, y_train, y_test = train_test_split(X_Seq, y_Seq, test_size=0.2,random_state=1675)
